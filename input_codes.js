...
function isTargetDate(tarDayWeek, tarDate) {
    const _isTargetDate = (JapaneseHolidays.isHoliday(tarDate) == undefined
        && tarDayWeek.indexOf(tarDate.getDay()) != -1)
    return _isTargetDate
}

/** 交通費自動申請 */
function inputTransportExpenses(tarYearVal, tarMonthVal, tarDayWeek,
                                expItemVal, startStVal, endStVal, changeStVal,
                                aroundFlg, jobVal, jobTextVal) {
    // 月の日数
    var daysInMonth = ((new Date(tarYearVal, tarMonthVal, 0)
        - (new Date(tarYearVal, tarMonthVal - 1, 1))) / 86400000) + 1
    // 日付を進めるために使用
    var days_counter = 1
    // ループ回数の上限
    ...
    var timerid = setInterval(function() {
        ...
        if (days_counter == daysInMonth + 1 || loop_counter == loop_max_count) {
            ...
            if (!ekitan_searching && !change_searching) {
                ...
                //  利用日
                var tarDateVal = tarYearVal + '/' + zeroPadding(tarMonthVal, 2) + '/' + zeroPadding(days_counter, 2)
                ...
                //  往復フラグ
                if (aroundFlg) {
                    document.getElementsByClassName('pp_base ts-form-roundtrip pp_btn_oneway')[0].click()
                }

                //  ジョブ
                var jobEl = document.getElementById('DlgDetailJob')
                jobEl.value = jobVal
                var jobTextEl = document.getElementsByClassName('ts-row-job')[0].getElementsByTagName('input')[0]
                ...
                if (!changeStVal && document.getElementsByClassName("station").length != 0) {
                    console.log("############ 駅探画面 ###############")
                    // 駅探処理が終わりなのでステータスを戻す
                ...
                } else if (!!changeStVal && document.getElementsByClassName("station").length != 0) {
                    console.log("############ 駅探画面 ###############")
                    // 経由地検索し駅探処理継続
            ...
            } else if (change_searching && document.getElementById("dijit_Dialog_0").style.display == 'none') {
...
function inputWorkloadPerformance(job_clock_map) {
    ...
    var _written_day_borders = Array.from(_day_borders).filter(function(item) {
        return item.getElementsByClassName("png-add").length != 0 && !!item.getElementsByClassName("time")[0].innerText.match(clockrate_re)
    })
    ...
    var timerid = setInterval(function() {
        ...
        if (days_counter == active_day_borders.length || loop_counter == loop_max_count) {
            clearInterval(timerid)
            console.log("Finished")
            console.log("daysCounter: "+days_counter)
            console.log("loopCounter: "+loop_counter)
        } else if (active_day_borders[days_counter].getElementsByClassName("time")[0].innerText != ""
                     && active_day_borders[days_counter].getElementsByClassName("time")[0].innerText != Object.values(job_clock_map)[0]
                || !!active_day_borders[days_counter].getElementsByClassName("time")[0].innerText.match(clockrate_re)) {
            // job_clock_map の初期値に一致しない値、もしくは %表示ならスルーしてdays_counter進める
            days_counter++
            loop_counter++
            wl_entering = false            
        } else if (!wl_entering && active_day_borders[days_counter].getElementsByClassName("time")[0].innerText == "") {
            // 工数登録画面on
            active_day_borders[days_counter].getElementsByClassName("png-add")[0].click()
            // クロック入力を行う
            Object.keys(job_clock_map).forEach(key => {
                if (!document.getElementById("empWorkLock"+key).className.includes("pb_btn_clockon")) {
                    // クロック入力on
            ...
            if (!!active_day_borders[days_counter].getElementsByClassName("time")[0].innerText.match(clockon_re)) {
                wl_entering = false
            } else if (!!active_day_borders[days_counter].getElementsByClassName("time")[0].innerText.match(clockrate_re)) {
                days_counter++
                wl_entering = false
            }
...
function inputAttendanceFix(isLightning, begin_time, end_time, until_latest) {
    ...
    var no_entries = function() {
        ...
        return Array.from(document.getElementsByClassName("png-add")).filter(function(item) {
            var tar_date_str = item.parentElement.id
            const tar_date = new Date(tar_date_str.slice(8))
            const day_times = item.parentElement.parentElement.getElementsByClassName("dval day_time0")
            if (until_latest) {
                // 最新日付までを修正する
                return (item.parentElement.parentElement.className.includes("days odd")
                            || item.parentElement.parentElement.className.includes("days even")) // even or odd が所定労働日
    ...
    var timerid = setInterval(function() {
        if (no_entries().length == 0 || loop_counter == loop_max_count) {
            ...
            // 値入力
            if (!document.getElementsByClassName("inputime roundBegin inputab")[0].value) {
                document.getElementsByClassName("inputime roundBegin inputab")[0].value = convertRandomFactor(begin_time, "begin")
            }
            if (!document.getElementsByClassName("inputime roundEnd inputab")[0].value) {
                document.getElementsByClassName("inputime roundEnd inputab")[0].value = convertRandomFactor(end_time, "end")
            }

            document.getElementById("empApplyDone1").click()
            ad_entering = true
        } else if (!is_first && ad_entering && no_entries().length != pre_no_entries.length) {
            // 通信完了
            ad_entering = false
            pre_no_entries = no_entries()
        } else {
...
function setupLib() {
    ...
    appendScript(src)
    // jpholiday
    src = "https://cdn.rawgit.com/osamutake/japanese-holidays-js/v1.0.9/lib/japanese-holidays.min.js"
    appendScript(src)
...
function execTransport(isLightning) {
    ...
    document.sfDSAutoForm.sfDSAutoButton.addEventListener('click', function() {
        ...
        // 経費のどの項目か
        const expItemVal  = 'a0Y7F000000h4RUUAY'
        ...
        inputTransportExpenses(tarYearVal, tarMonthVal, tarDayWeek,
                                expItemVal, startStVal, endStVal, changeStVal,
                                aroundFlg, jobVal, jobTextVal)
    });
}
...
function createWorkloadDSForm(isLightning) {
    if (isLightning) {
        document.getElementById('expTopView').insertAdjacentHTML('beforebegin','<div style="background-color:#66CCFF" class="DSTeamAutomation"><h1>工数実績設定</h1><p>適当な値でなんやかんやと計算をしているのでピッタリにならない<p>月の途中でJOBが変わるパターンは未想定（そのうち）<p><form name="sfDSAutoForm"><div class="DSJobList"></div></p><p><button name="sfDSAutoButton" type="button">入力</button></p></form></div>')
    } else {
        document.getElementById('AppBodyHeader').insertAdjacentHTML('beforebegin','<div class="DSTeamAutomation"><h1>工数実績設定</h1><p>適当な値でなんやかんやと計算をしているのでピッタリにならない<p>月の途中でJOBが変わるパターンは未想定（そのうち）<p><form name="sfDSAutoForm"><div class="DSJobList"></div></p><p><button name="sfDSAutoButton" type="button">入力</button></p></form></div>')
    }
    // JOBリストを取得してその数だけフォーマットを用意
    // 入力済みのJOB以降を取得
    var day_borders = document.getElementsByClassName("day_border")
    Array.from(day_borders).filter(function(item) {
        return item.getElementsByClassName("png-add").length != 0 && item.getElementsByClassName("time")[0].innerText == ""
    })[0].getElementsByClassName("png-add")[0].click()
    ...
    for(var i = 0; i < rowList.length; i++) {
        ...
        document.getElementsByClassName("DSJobList")[0].insertAdjacentHTML(
            'beforeend',
            '<p><label for="sfDSJob">'+jobRowValues[1]+': <input id="'+jobRowValues[0]+'_'+'sfDSJob" type="number" value=0></input>'+'%'+'</label></p>'
        )
    }
...
function createAttendanceDSForm(isLightning) {
    if (isLightning) {
        ...
        document.getElementById('AppBodyHeader').insertAdjacentHTML('beforebegin','<div class="DSTeamAutomation"><h1>勤務時間設定(例: 9:00, 18:00)</h1><p>当日日付までの承認済み/承認待ちでない日の勤怠打刻修正を、値のない箇所のみ修正します。</p><br><p>ランダムで20分まで前後します。</p><form name="sfDSAutoForm"><div class="DSAttendancBegin"><label for="DSAttendanceBeginTime">勤務開始時間: <input id="DSAttendanceBeginTime" type="text" value="10:00"></input></label></div><div class="DSAttendancEnd"><label for="DSAttendanceEndTime">勤務終了時間: <input id="DSAttendanceEndTime" type="text" value="19:00"></input></label></div><div><lavel for="DSUntileLatestCheck">今日分までの修正<input id="DSUntileLatestCheck" type="checkbox" checked="checked"></input></lavel></div></p><p><button name="sfDSAutoButton" type="button">入力</button></p></form></div>')
    }
    return true
}
...
function execAttendance(isLightning) {
    ...
    document.sfDSAutoForm.sfDSAutoButton.addEventListener('click', function() {
        ...
        attend_re = /^\d{1,2}:\d{2}$/
        if (!begin_time.match(attend_re)
                || !end_time.match(attend_re)) {
...
(function() {
    ...
    const isLightning = location.href.includes("lightning") || location.href.includes("visualforce.com")
    ...
    // 社名を隠すためにURLから引っ張る
    const PREFIX = location.href.slice(location.href.indexOf("/")+2, location.href.indexOf("."))

    switch (app_id) {
        // workload
        // lightning
        case "AtkEmpJobTab":
            alert("専用ページ(iframe)に遷移します。\nページが更新されたら再度ブックマークレットを実行してください。")
            location.href = "https://" + PREFIX + LIGHTNING_BASE_URI + "/apex/AtkEmpJobView"
            return
        // classic
        case "AtkEmpJobView":
        ...
        case "AtkEmpExpTab":
            alert("専用ページ(iframe)に遷移します。\nページが更新されたら再度ブックマークレットを実行してください。")
            location.href = "https://" + PREFIX + LIGHTNING_BASE_URI + "/apex/AtkEmpExpView"
            return
        // classic
        case "AtkEmpExpView":
        ...
        case "AtkWorkTimeTab":
            alert("専用ページ(iframe)に遷移します。\nページが更新されたら再度ブックマークレットを実行してください。")
            location.href = "https://" + PREFIX + LIGHTNING_BASE_URI + "/apex/AtkWorkTimeView"
            return
        // classic
